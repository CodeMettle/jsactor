...
object JsInternalActorContext {
  private[jsactor] case class Envelope(message: Any, sender: JsActorRef)
...
class JsInternalActorContext(override val props: JsProps, override val parent: JsActorRef, _actorRef: JsInternalActorRef)
  ...
  private[jsactor] var _currentMessage: Envelope = _

  private[jsactor] var _mailbox = Vector.empty[Envelope]
