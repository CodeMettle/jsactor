...
object JsActorSystem {
    ...
    private class MinimalActRef(override val path: JsActorPath) extends JsActorRef with WebLogging {
        override def !(message: Any)(implicit sender: JsActorRef): Unit = {
            logger.warn(s"Got $message from $sender")
        }
    }
    ...
    private class UserGuardian(onStop: Promise[Unit]) extends JsActor with WebLogging {
        ...
        def receive = {
            case msg ⇒ logger.warn(s"$msg sent to ${self.path}")
        }
    }
}

final case class JsActorSystem(name: String)
                              (implicit val dispatcher: ExecutionContextExecutor = JSExecutionContext.queue)
    extends JsActorRefFactory with Watcher with WebLogging {
    ...
    val eventStream = new JsEventStream
